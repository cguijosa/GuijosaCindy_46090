...
void jackTen(float &,float &,unsigned short &);
void waistIn(float &, unsigned short &);
void waisTen(float &, unsigned short &);
 ...
 * *************************jackTen********************************
 * ***************************************************************
 * Purpose: To calculate jacket size in 10 years and output result
 * Input:   
 *         weight->weight of user
 *         height->height of user
 *         age->age of user
 * Output:
 *         jackSiz->The jacket size for user under 30 years old in 10 years
 *         totjack->The jacket size for user over 30 years old in 10 years
 * **************************************************************/

void jackTen(float &weight,float &height,unsigned short &age){
    if(age+10<40){
      float jackSiz=(height*weight)/288.0f;
      cout<<setprecision(3)<<fixed;
      cout<<"In 10 years your jacket size will be "<<jackSiz<<endl;
    }else{
      float jackSiz=(height*weight)/288.0f;
      float temp=((age+10)-30.0f)/10.0f;//To calculate how many times I have to add 1/8th of an inch
      static_cast<int>(temp);//To get rid of decimals in between 10 years
      float totjack=jackSiz+(temp*0.125);//Adding 1/10 of an inch every 2 years
      cout<<setprecision(3)<<fixed;
      cout<<"In 10 years your jacket size will be "<<totjack<<endl;
    }
}
 ...
 * *************************waisTen*******************************
 * ***************************************************************
 * Purpose: To calculate waist in inches after 10 years
 * Input:   
 *         weight->weight of user
 *         age->age of user
 * Output:
 *         waist->The waist size for user under 28 years old in 10 years
 *         totwais->The jacket size for user over 28 years old in 10 years
 * **************************************************************/

void waisTen(float &weight, unsigned short &age){
    if(age+10>=30){
        float waist=weight/5.7f; 
        float temp=((age+10)-28.0f)/2.0f;//To calculate how many times I have to add 1/10th of an inch
        static_cast<int>(temp);//To get rid of decimal in between 2 years
        float totwais=waist+(temp*0.1f);//Adding 1/10 of an inch every 2 years
        cout<<setprecision(3)<<fixed;
        cout<<"In 10 years your waist size will be "<<totwais<<" (In) "<<endl;
    }else{
        float waist=weight/5.7f; 
        cout<<setprecision(3)<<fixed;
        cout<<"In 10 years your waist size will be "<<waist<<" (In) "<<endl;
        }
}
